<?php $__env->startSection('title', 'Pusat Peringatan Sistem'); ?>
        ...
        <div class="px-lg py-sm bg-surface-container-low border border-outline-variant/30 rounded-xl text-right">
            ...
            <?php if($activeCount > 0): ?>
                <p class="font-headline-sm text-headline-sm text-error flex items-center justify-end gap-2 font-semibold animate-pulse">
                    ...
                    ANOMALI AKTIF (<span id="live-active-count"><?php echo e($activeCount); ?></span>)
                </p>
            <?php else: ?>
        ...
        <div class="col-span-12 lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-gutter">
            <div class="bg-surface-container border border-outline-variant/30 p-lg rounded-xl flex flex-col justify-between relative overflow-hidden group hover:border-error/40 transition-colors">
                ...
                <h3 class="font-display-lg text-display-lg text-error mt-xs font-bold shadow-[0_0_15px_rgba(255,180,171,0.2)]" id="hero-critical-count">
                    <?php echo e($incidents->where('status', 'aktif')->where('jenis_insiden', 'Tidak Layak Pakai')->count()); ?>
            ...
            <div class="bg-surface-container border border-outline-variant/30 p-lg rounded-xl flex flex-col justify-between hover:border-primary/40 transition-colors">
                ...
                ?>
                <h3 class="font-display-lg text-display-lg text-primary mt-xs font-bold" id="live-health"><?php echo e(number_format($health, 1, ',', '.')); ?>%</h3>
                <div class="w-full h-1 bg-surface-variant rounded-full mt-md overflow-hidden">
                    <div class="h-full bg-primary shadow-[0_0_8px_rgba(76,215,246,0.4)]" style="width: <?php echo e($health); ?>%" id="health-bar"></div>
                </div>
            </div>
            <div class="bg-surface-container border border-outline-variant/30 p-lg rounded-xl flex flex-col justify-between hover:border-primary/40 transition-colors">
            ...
            <div class="relative z-10 p-md flex flex-col h-full justify-between">
                ...
                <div class="space-y-2">
                    <div class="flex items-center justify-between bg-background/75 backdrop-blur-sm p-2 rounded border border-outline-variant/20">
                        ...
                        <span class="text-on-surface-variant text-[10px] font-semibold" id="active-incident-badge"><?php echo e($activeCount); ?> INSIDEN AKTIF</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        ...
        <div class="lg:col-span-8 space-y-md">
            ...
            <div class="space-y-sm" id="live-alert-feed">
                <?php $__empty_1 = true; $__currentLoopData = $incidents->where('status', 'aktif'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $incident): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div id="incident-card-<?php echo e($incident->id); ?>" class="bg-surface-container/60 backdrop-blur-md p-md rounded-lg flex items-center gap-md border border-outline-variant/30 <?php echo e($incident->jenis_insiden === 'Tidak Layak Pakai' ? 'border-l-4 border-l-error hover:border-error/50 ring-1 ring-error/20' : 'border-l-4 border-l-tertiary hover:border-tertiary/50'); ?> transition-all duration-300">
                    <div class="w-10 h-10 rounded flex items-center justify-center shrink-0 <?php echo e($incident->jenis_insiden === 'Tidak Layak Pakai' ? 'bg-error/10 text-error shadow-[0_0_8px_rgba(239,68,68,0.3)] animate-pulse' : 'bg-tertiary/10 text-tertiary shadow-[0_0_8px_rgba(255,185,95,0.2)]'); ?>">
                        <span class="material-symbols-outlined text-[20px]">
                            <?php echo e($incident->jenis_insiden === 'Tidak Layak Pakai' ? 'error' : 'warning'); ?>

                        </span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-start">
                            <h5 class="font-body-md text-body-md font-bold <?php echo e($incident->jenis_insiden === 'Tidak Layak Pakai' ? 'text-error' : 'text-tertiary'); ?>">
                                <?php echo e($incident->jenis_insiden); ?> (BOX-<?php echo e($incident->perjalananRute->id_box ?? 'N/A'); ?>)
                            </h5>
                            <span class="text-on-surface-variant font-data-mono text-data-mono text-xs"><?php echo e($incident->created_at->format('H:i:s')); ?></span>
                        </div>
                        <p class="text-on-surface text-xs mt-1"><?php echo e($incident->deskripsi); ?></p>
                        <div class="flex items-center gap-4 mt-2 text-[10px] text-outline">
                            <span>Suhu Tercatat: <span class="font-data-mono font-bold text-on-surface"><?php echo e(number_format($incident->suhu_tercatat, 1, ',', '.')); ?>°C</span></span>
                            <span>Durasi Ekskursi: <span class="font-data-mono font-bold text-on-surface"><?php echo e($incident->durasi_anomali); ?> detik</span></span>
                        </div>
                    </div>
                    <div class="flex gap-xs">
                        <button class="btn-view-log px-md py-sm bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-on-surface rounded border border-outline-variant/30 transition-all font-label-md text-[10px] font-bold"
                                data-id="#BG-<?php echo e($incident->id); ?>"
                                data-type="<?php echo e($incident->jenis_insiden); ?>"
                                data-level="<?php echo e($incident->jenis_insiden === 'Tidak Layak Pakai' ? 'KRITIS' : 'PERINGATAN'); ?>"
                                data-duration="<?php echo e($incident->durasi_anomali); ?> detik"
                                data-temp="<?php echo e(number_format($incident->suhu_tercatat, 1, ',', '.')); ?>°C"
                                data-desc="<?php echo e($incident->deskripsi); ?>"
                                data-time="<?php echo e($incident->created_at->format('Y-m-d H:i:s')); ?>"
                                data-status="aktif">
                            DETAIL
                        </button>
                        <button onclick="resolveIncident(<?php echo e($incident->id); ?>)" class="px-md py-sm bg-primary/10 text-primary hover:bg-primary/20 rounded border border-primary/20 transition-all font-label-md text-[10px] font-bold">
                            KONFIRMASI
                        </button>
                    </div>
                </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="text-sm divide-y divide-outline-variant/10" id="resolved-incidents-table">
                    <?php $__empty_1 = true; $__currentLoopData = $incidents->where('status', 'resolved'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $incident): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-primary/5 transition-colors group">
                        <td class="px-lg py-md font-data-mono text-on-surface">#BG-<?php echo e($incident->id); ?></td>
                        <td class="px-lg py-md font-bold text-on-surface"><?php echo e($incident->jenis_insiden); ?></td>
                        <td class="px-lg py-md">
                            <?php if($incident->jenis_insiden === 'Tidak Layak Pakai'): ?>
                        ...
                        <td class="px-lg py-md font-data-mono text-on-surface-variant"><?php echo e($incident->durasi_anomali); ?> detik</td>
                        <td class="px-lg py-md font-data-mono text-on-surface-variant"><?php echo e(number_format($incident->suhu_tercatat, 1, ',', '.')); ?>°C</td>
                        <td class="px-lg py-md">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\project pkm\bio_guard_backend\resources\views\dashboard\alerts.blade.php ENDPATH**/ ?>