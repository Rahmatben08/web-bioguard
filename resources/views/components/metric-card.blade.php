...
<x-card class="relative flex flex-col justify-between transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-md hover:border-{{ $color }}/50">
    ...
    <div class="flex items-end gap-3 mt-1">
        <span {{ $valueId ? 'id='.$valueId : '' }} class="text-4xl font-extrabold tracking-tight text-slate-900  tabular-nums {{ $valueClass }}">{{ $value }}</span>
        
        @if($trend)
            <div class="flex items-center gap-0.5 mb-1 text-[11px] font-bold {{ $trendUp ? 'text-green-600 ' : 'text-red-600 ' }}">
