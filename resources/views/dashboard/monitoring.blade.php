...
<div class="flex-1 w-full min-h-full p-container-margin space-y-lg">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-gutter">
        ...
        <x-kpi-card title="Kurir Aktif" value="{{ $totalKurirAktif ?? 0 }}" icon="local_shipping" color="sky" subtitle="Rute Perjalanan Aktif" />

        {{-- Card 2: Pending Sync --}}
        <x-kpi-card title="Sinkronisasi Offline" value="{{ $totalPendingSync ?? 0 }}" icon="sync_problem" color="amber" subtitle="Menunggu Sinyal" />

        {{-- Card 3: System Status --}}
        <x-kpi-card title="Status Peringatan" value="{{ $alertCount ?? 0 }}" icon="{{ ($alertCount ?? 0) > 0 ? 'warning' : 'verified_user' }}" color="{{ ($alertCount ?? 0) > 0 ? 'rose' : 'emerald' }}" subtitle="{{ ($alertCount ?? 0) > 0 ? 'Segera Tindak Lanjuti!' : 'Semua Sistem Aman' }}" />
    </div>
...
(function () {
    ...
    // Planned Reference Routes — fetched dynamically via OSRM
const plannedPaths = {};
    ...
    const bootstrapData = initialList.map(p => {
        ...
        const destCoord = {lat: parseFloat(p.dest_latitude) || -2.9865, lng: parseFloat(p.dest_longitude) || 104.7522};
    ...
    window.triggerDashboardSimulation = function(type) {
        ...
        if (routeTrails[activeRouteId] && routeTrails[activeRouteId].length > 0) {
            ...
            if (currentRouteData && currentRouteData.dest_latitude) {
                lat = parseFloat(currentRouteData.dest_latitude);
                lng = parseFloat(currentRouteData.dest_longitude);
            }
        }
    ...
    window.downloadOfficialPdf = function () {
        ...
        window.open('{{ route("dashboard.audit-pdf") }}' + currentSearch, '_blank');
