                            ...
                            <div class="flex items-end justify-between mt-sm">
                                <div>
                                    ...
                                    <p class="text-2xl font-extrabold tracking-tight {{ $textClass }} tabular-nums">
                                        ...
                                        @endif
                                        {{ $temp !== null ? number_format($temp, 1, ',', '.') . 'Ã‚Â°C' : '-' }}
                                    </p>
                                </div>
                                <div class="text-right">
                                    ...
                                    <p class="text-base font-bold text-on-surface-variant tabular-nums">
                                        {{ is_numeric($mkt) ? number_format($mkt, 1, ',', '.') . 'Ã‚Â°C' : $mkt }}
                                    </p>
                                </div>
                            </div>
            ...
            <div>
                ...
                <div class="p-6">
                    <p class="text-xs text-slate-500 dark:text-on-surface-variant mb-4 leading-relaxed">
                        Kargo obat termolabil yang terdeteksi melanggar batas toleransi suhu dingin (anomali suhu &gt; 8Ã‚Â°C selama &gt; 30 detik) secara otomatis dialihkan ke status Karantina untuk pengujian laboratorium lanjutan sebelum pembuangan.
                    </p>
                        ...
                        <table class="w-full text-left border-collapse text-xs">
                            ...
                            <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60">
                                ...
                                <tr class="hover:bg-slate-100/50 dark:hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center text-error font-bold font-mono">10,2Ã‚Â°C</td>
                                ...
                                <tr class="hover:bg-slate-100/50 dark:hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center text-error font-bold font-mono">9,5Ã‚Â°C</td>
                                    <td class="px-4 py-3 text-right">
                                        <x-badge color="error" class="animate-pulse">
                                            Tidak Layak Pakai
                                        </x-badge>
                                ...
                                <tr class="hover:bg-slate-100/50 dark:hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center text-error font-bold font-mono">8,9Ã‚Â°C</td>
                                    <td class="px-4 py-3 text-right">
                                        <x-badge color="error" class="animate-pulse">
                                            Tidak Layak Pakai
                                        </x-badge>
                ...
                <div class="grid grid-cols-2 gap-4 text-xs">
                    ...
                    <div class="col-span-2">
                        ...
                        <p class="text-[11px] text-slate-650 dark:text-on-surface-variant leading-relaxed font-semibold mt-0.5">
                            Semua boks penyimpanan aktif terpantau berada dalam standar rantai dingin (2,0Ã‚Â°C - 8,0Ã‚Â°C) dengan fluktuasi rata-rata <span class="text-primary font-bold">4,8Ã‚Â°C</span> tanpa kerusakan zat aktif terdeteksi.
                        </p>
                    </div>
                    <div class="col-span-2">
...
const plannedPaths = {
    ...
    function createPopupContent(c) {
        ...
        const tempDisplay = temp !== null ? temp.toFixed(1).replace('.', ',') + 'Ã‚Â°C' : '-';
    ...
    function renderTelemetryCards(list) {
        ...
        list.forEach(c => {
            ...
            const duration = c.excursion_duration;
            const temp = c.suhu_aktual;
            const mkt = c.nilai_mkt !== null ? c.nilai_mkt.toFixed(1).replace('.', ',') + 'Ã‚Â°C' : '-';
            const prob = c.probabilitas_rusak;
        ...
        if (window.Notification && Notification.permission === 'granted') {
            try {
                new Notification('ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ALARM SUHU KRITIS BIO-GUARD', {
                    body: `Kondisi kritis pada Kurir ${c.nama_kurir} (${c.id_box})! Suhu saat ini: ${c.suhu_aktual.toFixed(1).replace('.', ',')}Ã‚Â°C.`,
                    icon: '/favicon.ico'
                });
            } catch (e) {
    ...
    function processLiveData(list, stats) {
        ...
        filteredList.forEach(c => {
            ...
            if (c.excursion_status !== previousStatuses[ruteId]) {
                if (c.excursion_status === 'Tidak Layak Pakai') {
                    logGatewayActivity('TG', 'Suhu', `Bot Telegram: Alert dikirim ke Dispatcher. Kargo BOX-${c.id_box} (${c.nama_kurir}) SUHU KRITIS ${c.suhu_aktual.toFixed(1).replace('.', ',')}Ã‚Â°C!`, 'danger');
                    logGatewayActivity('WA', 'Suhu', `WhatsApp Gateway (${c.no_wa}): Peringatan dikirim ke Kurir ${c.nama_kurir}. Boks ${c.id_box} melebihi batas aman: ${c.suhu_aktual.toFixed(1).replace('.', ',')}Ã‚Â°C. Pindahkan kargo segera!`, 'danger');
                } else if (c.excursion_status === 'Peringatan') {
                    logGatewayActivity('TG', 'Suhu', `Bot Telegram: Peringatan dini dikirim ke Dispatcher. Boks ${c.id_box} mendeteksi anomali suhu ${c.suhu_aktual.toFixed(1).replace('.', ',')}Ã‚Â°C.`, 'warning');
            ...
            if (c.excursion_status === 'Tidak Layak Pakai') {
                addNotification(`${c.id_box}-temp-danger`, `SUHU KRITIS: ${c.id_box}`, `Suhu kargo kurir ${c.nama_kurir} terdeteksi ${c.suhu_aktual.toFixed(1).replace('.', ',')}Ã‚Â°C melebihi batas cold chain!`, 'danger');
            } else if (c.excursion_status === 'Peringatan') {
                addNotification(`${c.id_box}-temp-warning`, `Peringatan: ${c.id_box}`, `Terdeteksi anomali suhu jangka pendek ${c.suhu_aktual.toFixed(1).replace('.', ',')}Ã‚Â°C.`, 'warning');
            }
            ...
            <div class="bg-white dark:bg-slate-900 border border-outline-variant/30 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl scale-95 opacity-0 transition-all duration-300 flex flex-col animate-fade-slide-in">
                ...
                <div class="p-6 space-y-4">
                    <div class="p-4 rounded-xl border border-green-500/30 bg-green-500/10 flex flex-col gap-2">
                        ...
                        <div class="grid grid-cols-2 gap-sm mt-1 text-[10px] font-mono text-on-surface-variant">
                            <div>Suhu Tiba: <span class="text-green-500 font-bold">${temp !== null ? temp.toFixed(1).replace('.', ',') + 'Ã‚Â°C' : '-'}</span></div>
    ...
    const bootstrapData = initialList.map(p => {
        ...
        let status = 'Aman';
        let statusLabel = 'Aman (Sesuai Standar 2Ã‚Â°C - 8Ã‚Â°C)';
    ...
    window.triggerDashboardSimulation = function(type) {
        ...
        if (type === 'suhu') {
            temp = 9.8;
            message = `Simulasi lonjakan suhu kritis (${temp.toFixed(1).replace('.', ',')}Ã‚Â°C) dikirim untuk Boks ${activeBoxId}`;
        } else if (type === 'deviasi') {
            lat = lat - 0.008; 
            lng = lng + 0.012;
            ...
            temp = 4.2;
            message = `Simulasi reset kondisi normal (suhu ${temp.toFixed(1).replace('.', ',')}Ã‚Â°C) dikirim untuk Boks ${activeBoxId}`;
        }
        ...
        yaxis: {
            labels: {
                formatter: function (val) {
                    return val.toFixed(1).replace('.', ',') + 'Ã‚Â°C';
                },
                style: {
                    colors: themeColors.textColor,
                    fontSize: '10px',
